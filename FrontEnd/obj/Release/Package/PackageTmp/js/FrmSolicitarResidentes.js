...
function cargarCarreras() {
    $.ajax({
        type: 'POST',
        url: 'ws/WSCarreras.asmx/getCarreras',
        contentType: 'application/json; utf-8',
        dataType: 'json',
        success: function (data) {
            let carreras = JSON.parse(data.d);
            $('#contenido_ddlCarrera').empty();
            for (i = 0; i < carreras.length; i++) {
                $('#contenido_ddlCarrera').append('<option value="' + carreras[i].carrera + '">' + carreras[i].carrera + '</option>');
            }
        },
        error: function (jqXHR, textStatus, errorThrown) {
            $("#msgError").text("HA OCURRIDO UN ERROR, POR FAVOR VUELVELO A INTENTAR" + textStatus);
            $("#mdlError").modal().show();
        }
    });
}
